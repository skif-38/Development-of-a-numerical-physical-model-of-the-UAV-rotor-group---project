...
// ---------- ПРАВИЛЬНЫЕ КОЭФФИЦИЕНТЫ (для абсолютных напряжений) ----------
const float vRef = 4.94;   // измерено мультиметром на 5V пине

const float ratio0 = 2.032;   // для A0 (точка B1)
const float ratio1 = 3.057;   // для A1 (точка B2)
const float ratio2 = 4.104;   // для A2 (точка B3)
const float ratio3 = 5.104;   // для A3 (точка B4)
// -----------------------------------------------------------------------
...
void loop() {
  ...
  if (currentMillis - prevSerialTime >= serialInterval) {
    ...
    // Вычисление напряжений каждой банки (разность между точками)
    float cell1 = vB1;
    float cell2 = vB2 - vB1;
    float cell3 = vB3 - vB2;
    float cell4 = vB4 - vB3;
